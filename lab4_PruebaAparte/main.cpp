...
void QuadTree::load(string filename){
...
void ObjectFile::compress(string filename){
...
void ObjectFile::decompress(string filename){
    fstream stream;
...
void ObjectFile::convertToPGM(string filename){
    fstream stream("out.qt", ios::in | ios::binary);
    if(stream.is_open()){
...
int main(){
    QuadTree test;
    ObjectFile stream;
    test.load("img/utec.pgm");
    stream = ObjectFile(test);
    stream.compress("out.qt");
    stream.decompress("out.qt");
    stream.convertToPGM("img/out.pgm");
}